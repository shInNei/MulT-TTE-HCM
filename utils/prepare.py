...
def MulT_TTE_collate_func(data, args, info_all):
    ...
    for ind, l in enumerate(data):
        ...
        inds.append(l[0])
    lens = np.asarray([len(k) for k in linkids], dtype=np.int32)
    ...
    rawlinks = np.full(mask.shape, fill_value=args.data_config['edges'] + 1, dtype=np.int32)
    ...
    def random_mask(tokens: np.array, rate: float):
        ...
        labels = np.full(len(tokens),dtype=np.int32, fill_value=-100)
        tokens = tokens.copy()
    ...
    mask_label = np.full(mask.shape, dtype=np.int32, fill_value=-100)
    mask_label[mask] = np.concatenate(mask_label_tmp)

    linkindex = np.full(mask.shape, fill_value=args.data_config['edges'] + 1, dtype=np.int32)
    linkindex[mask] = np.concatenate(sub_input_tmp)
    mask_encoder = np.zeros(mask.shape, dtype=np.int32)
